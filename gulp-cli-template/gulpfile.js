...
gulp.task('copyStatic', cb => {
  // 迁移第三方引用的库或插件
  pump([gulp.src('src/lib/**'), gulp.dest('dist/lib')])

  // 迁移字体
  pump([gulp.src('src/assets/fonts/**'), gulp.dest('dist/assets/fonts')])
...
gulp.task('server', () => {
  connect.server({
    host: 'localhost', // 本地host，默认为“localhost”
    port: 9001, // 端口
    root: 'dist', // 根目录
    ...
    middleware(connect, opt) {
      ...
      return [
        proxy('/api', {
          target: 'https://www.easy-mock.com', // 跨域指向
          changeOrigin: true
        })
      ]
      ...
      //     changeOrigin: true
      //   }
      //   return prefixs.map(item => proxy(item, config))
      ...
      //   }]
      //   return config.map(item => proxy(item.prefix, { item.target, changeOrigin }))
    }
  })
})
